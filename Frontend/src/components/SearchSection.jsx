...
export default function SearchSection() {
    ...
    useEffect(() => {
        setFilteredDepartureAirports(
            departureAirports.filter(airport => 
    ...
    useEffect(() => {
        setFilteredArrivalAirports(
            arrivalAirports.filter(airport => 
    ...
    useEffect(() => {
        if (multiCityFlights.length > 0) {
            setMultiCityDropdowns(multiCityFlights.map(() => ({
                showFrom: false,
                showTo: false,
                filteredFrom: [],
                filteredTo: []
        ...
        if (multiCityDropdowns.length > 0) {
            const updatedDropdowns = multiCityFlights.map((flight, index) => {
                return {
                    ...
                    filteredFrom: departureAirports.filter(airport => 
                        airport.toLowerCase().includes(flight.from.toLowerCase()) && airport !== flight.to
                    ),
                    filteredTo: arrivalAirports.filter(airport => 
                        airport.toLowerCase().includes(flight.to.toLowerCase()) && airport !== flight.from
                    )
                };
            });
    ...
    const handleSearch = (e) => {
        ...
        navigate("/search-results", { 
            state: { 
                tripType, from, to, departDate, returnDate, cabinClass 
            } 
        });
    };
    ...
    const flightFeatures = [
        { icon: <FaPlane />, text: "Explore the best flight deals from anywhere, to everywhere, then book with no fees" },
        { icon: <FaCalendarAlt />, text: "Compare flight deals from over 1000 providers, and choose the cheapest, fastest or lowest-emission tickets" },
        { icon: <FaTag />, text: "Find the cheapest month - or even day - to fly, and set up Price Alerts to book when the price is right" },
    ];
                        ...
                        <form className="space-y-4">
                            {multiCityFlights.map((flight, index) => (
                                <div key={flight.id} className="flex flex-wrap md:flex-nowrap gap-4 items-center">
                                    {/* From */}
                                    <div className="flex-1 min-w-[100px] relative" ref={el => {
                                        if (!multiCityRefs.current[index]) multiCityRefs.current[index] = {};
                                        ...
                                        <input 
                                            ...
                                            onChange={(e) => {
                                                ...
                                                setMultiCityFlights(updatedFlights);
                                                if (!multiCityDropdowns[index]?.showFrom) toggleMultiCityDropdown(index, 'from');
                                            }} 
                                            ...
                                            <div className="absolute z-50 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                                                {multiCityDropdowns[index]?.filteredFrom.length > 0 ? (
                                                    multiCityDropdowns[index].filteredFrom.map((airport, idx) => (
                                                        <div 
                                                            key={idx} 
                                                            className={`p-2 hover:bg-blue-100 cursor-pointer ${multiCityFocusIndices[index]?.from === idx ? 'bg-blue-200' : ''}`}
                                    ...
                                    <div className="flex-1 min-w-[100px] relative" ref={el => {
                                        ...
                                        <input 
                                            ...
                                            onChange={(e) => {
                                                ...
                                                setMultiCityFlights(updatedFlights);
                                                if (!multiCityDropdowns[index]?.showTo) toggleMultiCityDropdown(index, 'to');
                                            }}
                                            ...
                                            <div className="absolute z-50 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                                                {multiCityDropdowns[index]?.filteredTo.length > 0 ? (
                                                    multiCityDropdowns[index].filteredTo.map((airport, idx) => (
                                                        <div 
                                                            key={idx} 
                                                            className={`p-2 hover:bg-blue-100 cursor-pointer ${multiCityFocusIndices[index]?.to === idx ? 'bg-blue-200' : ''}`}
                                    ...
                                    <div className="absolute z-50 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                                        {filteredDepartureAirports.length > 0 ? (
                                            filteredDepartureAirports.map((airport, index) => (
                                                <div 
                                                    key={index} 
                                                    className={`p-2 hover:bg-blue-100 cursor-pointer ${fromFocusIndex === index ? 'bg-blue-200' : ''}`}
                                    ...
                                    <div className="absolute z-50 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                                        {filteredArrivalAirports.length > 0 ? (
                                            filteredArrivalAirports.map((airport, index) => (
                                                <div 
                                                    key={index} 
                                                    className={`p-2 hover:bg-blue-100 cursor-pointer ${toFocusIndex === index ? 'bg-blue-200' : ''}`}
                ...
                <div className="max-w-7xl container mx-auto text-center">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        <div className="bg-white p-6 rounded-lg shadow-md text-center hover:shadow-lg">
                            <img src="https://cdn-icons-png.flaticon.com/512/2645/2645568.png" alt="Globe" className="w-16 h-16 mx-auto mb-4" />
                            <h3 className="text-lg font-bold font-serif">Search ‘Everywhere’, explore anywhere</h3>
                            <p className="text-gray-600 mt-2 font-serif">Enter your departure airport and travel dates...</p>
                        </div>
                        <div className="bg-white p-6 rounded-lg shadow-md text-center hover:shadow-lg">
                            <img src="https://cdn-icons-png.flaticon.com/512/2353/2353181.png" alt="Transparent Pricing" className="w-16 h-16 mx-auto mb-4" />
                            <h3 className="text-lg font-bold font-serif">Pay less, go further with transparent pricing</h3>
                            <p className="text-gray-600 mt-2 font-serif">The cheapest flight deals. No hidden fees...</p>
                        </div>
                        <div className="bg-white p-6 rounded-lg shadow-md text-center hover:shadow-lg">
                            <img src="https://cdn-icons-png.flaticon.com/512/2529/2529521.png" alt="Price Alerts" className="w-16 h-16 mx-auto mb-4" />
                            <h3 className="text-lg font-bold font-serif">Book when it's best with Price Alerts</h3>
                            <p className="text-gray-600 mt-2 font-serif">Found your flight, but not quite ready to book...</p>
                        </div>
                    </div>
                </div>
            </section>
