...
const CarCard = ({ cars, locationState }) => {
  ...
  // Ensure up to 8 unique models for the active make are displayed
  const uniqueCars = [];
  ...
  if (uniqueCars.length < 8 && makeCars.length > 0) {
    console.warn(
      `Only ${uniqueCars.length} unique models found for ${activeMake}. Expected 8. Available data:`,
      makeCars.map((car) => `${car.make} ${car.model}`)
    );
  ...
  console.log(
    `Displayed Models for ${activeMake}:`,
    uniqueCars.map((car) => `${car.make} ${car.model}`)
  );
