...
// //     extras = {},
// //     totalPrice = 0, // Default to 0 if not present
...
//       try {
//         const response = await fetch('http://localhost:5008/api/save-booking', {
//           method: 'POST',
//           headers: {
...
const BookingConfirmation = () => {
  ...
  // Retrieve booking data from localStorage or location.state
  const storedBooking = localStorage.getItem('lastBooking');
  const bookingData = storedBooking ? JSON.parse(storedBooking) : location.state || {};
  ...
  const dropoffLoc = dropoffLocation || pickupLocation; // Simplified logic

  // Save booking to server and clean up
  useEffect(() => {
    const saveBookingToServer = async () => {
      try {
        const response = await fetch('http://localhost:5008/api/save-booking', {
  ...
  const downloadInvoice = () => {
    ...
    // Invoice Number and Booking Date
    const bookingNumber = Math.floor(1000000000 + Math.random() * 9000000000);
    doc.setFontSize(9);
    doc.setFont("helvetica", "normal");
    doc.text(`Invoice No: INV-2025-${bookingNumber}`, 190, yPosition, { align: "right" });
    yPosition += 5;
    ...
    doc.setFontSize(10);
    doc.text(`Traveler: Guest`, 15, yPosition);
    yPosition += 5;
    doc.text(`Email: Not provided`, 15, yPosition);
    yPosition += 5;
    doc.text(`Booking ID: TG-${bookingNumber}`, 15, yPosition);
    yPosition += 15;
    ...
    doc.setFontSize(12);
    doc.text("Additional Extras", 15, yPosition);
    doc.line(15, yPosition + 2, 75, yPosition + 2);
    yPosition += 10;
    ...
    doc.setFont("helvetica", "normal");
    doc.setTextColor(...darkGray);
    const baseCost = Math.round(totalPrice * 0.7) || 0;
    const extrasCost = Math.round(totalPrice * 0.3) || 0;
    const total = totalPrice || 0;
