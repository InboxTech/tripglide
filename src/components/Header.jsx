...
export default function Header() {
  ...
  const tabs = [
    ...
    { name: "carhire", icon: <FaCar />, path: "/carhire" },
  ];
  ...
  return (
    <div>
      ...
      <header className="flex justify-between items-center p-6 bg-[#06152B] text-white relative">
        {/* Logo and Title */}
        <div className="flex items-center gap-3 cursor-pointer" onClick={() => navigate("/carhire")}>
          <img src={logo} alt="Skyscanner Logo" className="h-10 w-auto" />
          <span className="text-2xl font-bold">Skyscanner</span>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-1 p-2 rounded-lg hover:bg-gray-600 transition-colors duration-300 cursor-pointer">
            <FaUser />
            <span>Log in</span>
          </div>
      ...
      <div className="flex gap-4 px-6 py-4 bg-[#06152B] text-white">
        {tabs.map((tab) => (
          <button
            key={tab.name}
            className={`flex items-center gap-2 px-4 py-2 rounded-full border border-white transition-transform duration-300 hover:scale-105 hover:bg-blue-600 ${
              location.pathname === tab.path ? "bg-blue-600" : "bg-gray-800"
            }`}
